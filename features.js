...
function findExByKey(key){
  const pk = parseExKey(key);
  if(pk.idx && pk.idx[0]==='a'){ // 自选添加的动作
...
document.addEventListener('click', e=>{
  ...
  if(dx){
    ...
    const ex = findExByKey(key);
    const pk = parseExKey(key);
    if(pk.idx[0]==='a'){ // 自选动作：直接从添加列表移除
      if(!confirm(`把「${ex?ex.n:key}」从计划中移除？\n这是你自己添加的动作，移除后如需恢复要重新添加。`)) return;
      const pa = store.planAdd; (pa[pk.addKey]||[]).splice(+pk.idx.slice(1),1);
      if(pa[pk.addKey] && !pa[pk.addKey].length) delete pa[pk.addKey];
      store.planAdd = pa;
      changed(); renderTrain();
      return;
    }
...
function localSummary(st){
  ...
  else if(st.pct>=50) tips.push(`训练完成率 ${st.pct}%，下周先保证主项训练日出勤`);
